        ...
        for it in range(it_x_ep):
            ...
            for task in range(t):  # for each task
                ...
                for name, param in model.named_parameters():  # for each weight
                    ...
                    loss += (fisher * (optpar - param).pow(
                        2)).sum() * ewc_lambda  # loss is accumulator # add penalty for current task and weight
...
def on_task_update(t, x, y, fisher_dict, optpar_dict, model, optimizer, criterion, mb_size, use_cuda=True, mask=None,
                   preproc=None):
    """
    INPUT:
